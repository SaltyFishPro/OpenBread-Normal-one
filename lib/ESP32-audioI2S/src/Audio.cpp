...
void Audio::setDefaults() {
    ...
    m_validSamples = 0;
    m_sbyt.noPcmDecodeCount = 0;
    m_sbyt.noPcmStartByte = 0;
...
int Audio::read_ID3_Header(uint8_t* data, size_t len) {
    ...
    if (m_controlCounter == MP3_XING) { //  exist another ID3tag?
        ...
        if ((*(data + 0) == 'I') && (*(data + 1) == 'D') && (*(data + 2) == '3')) {
            ...
            if (m_decoder) {
                int nextSync = m_decoder->findSyncWord(data, (int32_t)len);
                m_decoder->clear();
                if (nextSync > 0) {
                    m_audioDataStart += nextSync;
                    m_audioDataSize = m_audioFileSize - m_audioDataStart;
                    info(*this, evt_info, "MP3 audio sync shifted to %u", m_audioDataStart);
                    return nextSync;
                }
                if (nextSync < 0) {
                    uint32_t skip = ((uint32_t)len > 4096) ? 4096 : (uint32_t)len;
                    m_audioDataStart += skip;
                    m_audioDataSize = m_audioFileSize - m_audioDataStart;
                    return skip;
                }
            }
...
int Audio::sendBytes(uint8_t* data, size_t len) {
    ...
    constexpr uint16_t MP3_NO_PCM_MAX_DECODE_ATTEMPTS = 96;
    constexpr uint32_t MP3_NO_PCM_MAX_BYTES = 196608;
    ...
    auto                  noteMp3NoPcm = [&](uint32_t consumed) {
        if (m_codec != CODEC_MP3 || consumed == 0) return;
        if (m_dataMode != AUDIO_LOCALFILE && m_streamType != ST_WEBFILE) return;
        if (m_sbyt.noPcmDecodeCount == 0) { m_sbyt.noPcmStartByte = m_audioDataReadPtr; }
        m_sbyt.noPcmDecodeCount++;
        uint32_t noPcmBytes = m_audioDataReadPtr + consumed - m_sbyt.noPcmStartByte;
        if (m_sbyt.noPcmDecodeCount >= MP3_NO_PCM_MAX_DECODE_ATTEMPTS || noPcmBytes >= MP3_NO_PCM_MAX_BYTES) {
            AUDIO_LOG_ERROR("MP3 decode produced no PCM after %lu bytes", (long unsigned int)noPcmBytes);
            m_f_eof = true;
        }
    };
    ...
    if (res < 0) {
        uint32_t consumed = decodeError(res, data, bytesDecoded);
        noteMp3NoPcm(consumed);
        return consumed;
    } // Error, skip the frame...
    if (res > 99) {
        uint32_t consumed = decodeContinue(res, data, bytesDecoded, &m_sbyt.bytesLeft);
        noteMp3NoPcm(consumed);
        return consumed;
...
exit:
    ...
    if (m_validSamples) {
        ...
        playChunk();
    } else if (m_codec == CODEC_MP3 && bytesDecoded > 0) {
        noteMp3NoPcm(bytesDecoded);
    }
    return bytesDecoded;
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
uint8_t Audio::getSpectrum(uint8_t* out, uint8_t maxBands) {
    if (!out || maxBands == 0) return 0;

    const uint8_t available = (uint8_t)m_fft_items.BANDS;
    const uint8_t count = maxBands < available ? maxBands : available;
    if (!m_f_running) {
        for (uint8_t i = 0; i < count; i++) { out[i] = 0; }
        return count;
    }

    for (uint8_t i = 0; i < count; i++) { out[i] = m_fft_items.spectrum[i]; }
    return count;
}

uint8_t Audio::getSpectrumBandCount() const {
    return (uint8_t)m_fft_items.BANDS;
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
void Audio::processSpectrum() {
    ...
    constexpr float kBandEdges[FFT_BANDS + 1] = {
        40.0f,   230.0f,  430.0f,  650.0f,  900.0f,  1200.0f,
        1550.0f, 2000.0f, 2600.0f, 3400.0f, 4500.0f, 6000.0f,
        8000.0f, 10500.0f, 13500.0f, 17000.0f, 21000.0f};
    constexpr float kBandWeights[FFT_BANDS] = {
        1.35f, 1.25f, 1.20f, 1.35f, 1.55f, 1.70f, 1.65f, 1.55f,
        1.45f, 1.30f, 1.18f, 1.08f, 1.00f, 0.95f, 0.90f, 0.85f};
    ...
    for (int i = 1; i < m_fft_items.SIZE / 2; i++) {
        ...
        int b = -1;
        for (int bandIndex = 0; bandIndex < FFT_BANDS; bandIndex++) {
            if (f >= kBandEdges[bandIndex] && f < kBandEdges[bandIndex + 1]) {
                b = bandIndex;
                break;
            }
        }
    ...
    for (int i = 0; i < FFT_BANDS; i++) { band[i] *= kBandWeights[i]; }

    // log scale
    for (int i = 0; i < FFT_BANDS; i++) { band[i] = log10f(band[i] + 1e-6f); }

    // --- temporal smoothing (only displayed bands) ---
    auto smooth = [](float old, float in) {
        constexpr float ATTACK = 0.70f;
        constexpr float RELEASE = 0.48f;
        return (in > old) ? old + ATTACK * (in - old) : old + RELEASE * (in - old);
    };

    for (int i = 0; i < FFT_BANDS; i++) { m_fft_items.spec_smooth[i] = smooth(m_fft_items.spec_smooth[i], band[i]); }
